...
- **Main-thread `createSyncAccessHandle` is unreliable on iOS Safari 26.4 / iOS 18.7** — even when the API is exposed, `sync.write()` throws `"operation failed for an unknown transient reason"` on regular browser tabs, and the OPFS subsystem gets into a stuck state where subsequent file-handle creation also fails. The historical and currently-reliable home for sync-access-handle on Safari is a **Web Worker**, so `opfs.ts` now keeps `createWritable` on the main thread (Chromium / Firefox / Safari 18.4+ fast path) and delegates to `opfs-worker.ts` for the createSyncAccessHandle fallback. The worker tries three write-call shapes (`(view, {at:0})`, `(view)`, `(buffer, {at:0})`) because some Safari builds reject the options object. Probe diagnostics now report any caught errors instead of returning `null`, so the next round of debugging has more to go on if it's still broken.
